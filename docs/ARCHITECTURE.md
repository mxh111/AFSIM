...
- `app/services/afsim_maps.py`：只读服务化 AFSIM `resources/maps`，提供 MBTiles 瓦片、Blue Marble 地球贴图和 Shapefile GeoJSON。
...
- 地图底图已通过 `/api/afsim/maps` 服务化 AFSIM MBTiles 与 Shapefile；继续扩展 `resources/models/mil-std2525d` 的军标和模型加载。
